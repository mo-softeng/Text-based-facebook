...
{
    user_t *user_to_connect = find_user(users, friend);
    if (user_to_connect == NULL){
        printf("Friend not registered\n");
        return;
    }
    // Create a new friend node
    friend_t *new_friend = create_friend(friend);
    assert(new_friend != NULL);
    new_friend->posts = &(user_to_connect->posts);
...
void print_menu()
{
    ...
    printf("4. Display a user’s posts\n");
    printf("5. Exit\n");
...
bool valid_passowrd(user_t *users, char *passowrd) {
    for (user_t *cur = users; cur != NULL; cur = cur->next) {
        if (strcmp(passowrd, cur->password) == 0) {
            return 1;
        }
    }
    return 0;
